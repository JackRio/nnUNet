...
class nnUNetTrainerV2_Teacher(nnUNetTrainerV2):
    ...
    def initialize_network(self):
        if self.threeD:
            ...
            raise Exception("Currently only supports 3D")
...
class nnUNetTrainerV2_Student(nnUNetTrainerV2):
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, teacher_impact=0.8, **kwargs):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)

        self.teacher_output = None  # Will be populated after teacher runs
        self.teacher_impact = teacher_impact
    ...
    def run_iteration(self, data_dict, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():
                output = self.network(data)
                del data
                l = self.loss(output, target) + self.loss(output, self.teacher_output)
